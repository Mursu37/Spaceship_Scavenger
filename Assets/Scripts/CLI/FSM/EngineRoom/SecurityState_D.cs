...
namespace CLI.FSM
{
    public class SecurityState_D : State
    {
        public SecurityState_D(StateController controller) : base(controller)
        {
            directories.Add("access", new DoorState(controller));
            commands.Insert(1, "log");
            commands.Insert(2, "run diagnostics");
        }
          ...
          /*  else if (command == "override")
            ...
            else if (command == "run diagnostics")
            {
                stateController.ChangeText("SYSTEM_DIAGNOSTIC.exe executed.\r\n" +
                    ...
                    "<color=#c8a519>Warning: Door B07 is not responding to control commands. Manual Intervention required.\r\n");
            }
