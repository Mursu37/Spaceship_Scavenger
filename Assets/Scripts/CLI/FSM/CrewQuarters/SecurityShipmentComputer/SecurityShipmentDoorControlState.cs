...
namespace CLI.FSM
{
    public class SecurityShipmentDoorControlState : State
    {
        public SecurityShipmentDoorControlState(StateController controller) : base(controller)
        {
            commands.Insert(0, "a2");
        ...
        public override void Interpret(string command)
        {
            if (command == "a2")
            {
                stateController.ChangeText("Door A2 in lockdown. Submit access credentials \r\n" +
                    "<color=#3Ca8a8>[Module injecting false credentials...] [Success] \r\n" +
                    "<color=#c8a519>Door A2 unlocked and opened via terminal access");
